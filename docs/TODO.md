...
### Performance
- [ ] Cover image LRU memory cache — disk I/O on every cover request
- [ ] MagicShelfService.getBooksForShelf() — returns unbounded results, needs LIMIT
...
- [ ] seedFullDemo should auto-call seedFiles + seedLibrivox + seedComics
- [ ] Activity log page — persistent event history per user

### Testing
- [ ] Kobo/KOReader sync — tested with mocks only, never with real hardware
- [ ] OIDC/SSO — tested with mock provider, never with Keycloak/Auth0
