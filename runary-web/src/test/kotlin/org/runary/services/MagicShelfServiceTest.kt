...
class MagicShelfServiceTest {
    ...
    fun setup() {
        ...
        val result =
            authService.register(
                CreateUserRequest("shelf_${System.nanoTime()}", "shelf_${System.nanoTime()}@test.com", org.runary.TestPasswords.DEFAULT),
            )
    ...
    fun `getShelves returns empty for user with no shelves`() {
        val otherResult =
            authService.register(
                CreateUserRequest("shelfother_${System.nanoTime()}", "shelfother_${System.nanoTime()}@test.com", org.runary.TestPasswords.DEFAULT),
            )
    ...
    fun `getShelf returns null for wrong user`() {
        ...
        val otherResult =
            authService.register(
                CreateUserRequest("shelfwrong_${System.nanoTime()}", "shelfwrong_${System.nanoTime()}@test.com", org.runary.TestPasswords.DEFAULT),
            )
        val otherId = jwtService.extractUserId(otherResult.getOrThrow().token)!!
    ...
    fun `deleteShelf returns false for wrong user`() {
        ...
        val otherResult =
            authService.register(
                CreateUserRequest("shelfprot_${System.nanoTime()}", "shelfprot_${System.nanoTime()}@test.com", org.runary.TestPasswords.DEFAULT),
            )
        val otherId = jwtService.extractUserId(otherResult.getOrThrow().token)!!
