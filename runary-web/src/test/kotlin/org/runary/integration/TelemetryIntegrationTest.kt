...
class TelemetryIntegrationTest : IntegrationTestBase() {
        ...
        val loginResp =
            app(
                Request(Method.POST, "/auth/login")
                    ...
                    .body("""{"username":"${com.auth0.jwt.JWT.decode(token).getClaim("username").asString()}","password":"${org.runary.TestPasswords.DEFAULT}"}"""),
            )
        return Json.mapper.readValue(loginResp.bodyString(), org.runary.models.LoginResponse::class.java).token
    }
    ...
    fun `record only stores events for opted-in users`() {
        val token = registerAndGetToken()
        val jdbi =
            org.runary.TestFixture.database
                .getJdbi()
        val svc = TelemetryService(jdbi, org.runary.services.UserSettingsService(jdbi))
        val userId =
        ...
        val countBefore =
            jdbi.withHandle<Int, Exception> { h ->
                h
                    .createQuery("SELECT COUNT(*) FROM telemetry_events WHERE event_type = 'test.event'")
