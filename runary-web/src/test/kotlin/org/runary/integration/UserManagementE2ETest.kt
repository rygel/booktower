...
import org.runary.models.UserAdminDto
    ...
    fun `user can change password and login with new password`() {
        ...
        val oldLoginResp =
            app(
                Request(Method.POST, "/auth/login")
                    .header("Content-Type", "application/json")
                    .body(loginJson(username, org.runary.TestPasswords.DEFAULT)),
            )
        ...
        val changeResp =
            app(
                Request(Method.POST, "/api/auth/change-email")
                    .header("Cookie", "token=$token")
                    .header("Content-Type", "application/json")
                    .body("""{"currentPassword":"${org.runary.TestPasswords.DEFAULT}","newEmail":"$newEmail"}"""),
            )
