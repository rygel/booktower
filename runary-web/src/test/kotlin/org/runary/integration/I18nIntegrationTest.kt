...
class I18nIntegrationTest : IntegrationTestBase() {
    ...
    fun `English is default language`() {
        ...
        assertTrue(body.contains("Home") || body.contains("Runary"), "Expected English content")
    }
    ...
    fun `Spanish language renders translated content`() {
        ...
        assertTrue(body.contains("Runary"), "Page should still contain app name")
    ...
    fun `unsupported language falls back to English`() {
        ...
        val body = response.bodyString()
        assertTrue(body.contains("Runary"), "Page should render with fallback language")
    }
}
