...
class AdminIntegrationTest : IntegrationTestBase() {
    ...
    fun `admin page lists registered users`() {
        ...
        app(
            Request(Method.POST, "/auth/register")
                .header("Content-Type", "application/json")
                .body("""{"username":"$otherUsername","email":"$otherUsername@test.com","password":"${org.runary.TestPasswords.DEFAULT}"}"""),
        )
        val body =
            app(
                Request(Method.GET, "/admin")
    ...
    fun `admin cannot delete themselves`() {
        ...
        val loginResp =
            app(
                Request(Method.POST, "/auth/login")
                    .header("Content-Type", "application/json")
                    .body("""{"username":"admin_${System.nanoTime()}","password":"${org.runary.TestPasswords.DEFAULT}"}"""),
            )
