        ...
        val resp =
            app(
                Request(Method.POST, "/api/auth/change-password")
                    ...
                    .body("""{"currentPassword":"${org.runary.TestPasswords.DEFAULT}","newPassword":"newpassword456"}"""),
            )
        ...
        val resp =
            app(
                Request(Method.POST, "/api/auth/change-password")
                    .header("Cookie", "token=$token")
                    .header("Content-Type", "application/json")
                    .body("""{"currentPassword":"${org.runary.TestPasswords.DEFAULT}","newPassword":"abc"}"""),
            )
        assertEquals(Status.BAD_REQUEST, resp.status)
    }
        ...
        val resp =
            app(
                Request(Method.POST, "/api/auth/change-password")
                    .header("Cookie", "token=$token")
                    .header("Content-Type", "application/json")
                    .body("""{"currentPassword":"${org.runary.TestPasswords.DEFAULT}","newPassword":""}"""),
            )
        assertEquals(Status.BAD_REQUEST, resp.status)
    }
    ...
    fun `new password works for login after change`() {
        ...
        val token =
            org.runary.config.Json.mapper
                .readValue(registerResp.bodyString(), org.runary.models.LoginResponse::class.java)
                .token
        ...
        app(
            Request(Method.POST, "/api/auth/change-password")
                .header("Cookie", "token=$token")
                .header("Content-Type", "application/json")
                .body("""{"currentPassword":"${org.runary.TestPasswords.DEFAULT}","newPassword":"brandnewpass"}"""),
        )
        ...
        repeat(10) {
            app(
                Request(Method.POST, "/api/auth/change-password")
                    ...
                    .body("""{"currentPassword":"${org.runary.TestPasswords.DEFAULT}","newPassword":"newpass_$it"}"""),
            )
        }
        val resp =
            app(
