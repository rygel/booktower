    ...
    companion object {
        ...
        val postgres: PostgreSQLContainer<*> =
            PostgreSQLContainer("postgres:17.4-alpine")
                .withDatabaseName("runary_fts_http")
                .withUsername("test")
                .withPassword("test")
        ...
        fun setup() {
            ...
            val config = org.runary.TestFixture.config
            val templateRenderer = org.runary.TestFixture.templateRenderer
            ...
            val smtpConfig = org.runary.config.SmtpConfig("", 587, "", "", "", true)
            val authHandler =
                org.runary.handlers.AuthHandler2(
                    authService,
                    userSettingsService,
            ...
            val libraryHandler =
                org.runary.handlers.LibraryHandler2(libraryService, backgroundTaskService, config.storage)
            val bookHandler = org.runary.handlers.BookHandler2(bookService, readingSessionService)
            val bookmarkHandler = org.runary.handlers.BookmarkHandler(bookmarkService)
            val fileHandler =
                org.runary.handlers.FileHandler(
                    bookService,
                    pdfMetadataService,
                    epubMetadataService,
                    config.storage,
            ...
            val settingsHandler = org.runary.handlers.UserSettingsHandler(userSettingsService)
            val adminHandler =
                org.runary.handlers.AdminHandler(
                    adminService,
                    jwtService,
                    authService,
                    templateRenderer,
            ...
            val backgroundTaskHandler =
                org.runary.handlers.BackgroundTaskHandler(backgroundTaskService, seedService)
            val journalHandler = org.runary.handlers.JournalHandler(journalService)
            val opdsHandler =
                org.runary.handlers.OpdsHandler(
                    authService,
                    libraryService,
                    bookService,
                    config.storage,
            ...
            val apiTokenHandler = org.runary.handlers.ApiTokenHandler(apiTokenService, jwtService)
            val exportHandler = org.runary.handlers.ExportHandler(exportService, jwtService)
            val goodreadsImportHandler =
                org.runary.handlers.GoodreadsImportHandler(goodreadsImportService, jwtService)
            val bulkBookHandler = org.runary.handlers.BulkBookHandler(bookService)
            ...
            // Routers
            val weblateConfig = org.runary.config.WeblateConfig("", "", "", false)
            ...
            val libraryApiRouter =
                org.runary.routers.LibraryApiRouter(filters, libraryHandler, libraryService, null, null)
            val userApiRouter =
                org.runary.routers.UserApiRouter(
                    filters,
                    settingsHandler,
                    bookService,
                    userSettingsService,
            ...
            val adminApiRouter =
                org.runary.routers.AdminApiRouter(
                    filters,
                    adminHandler,
                    backgroundTaskHandler,
                    org.runary.weblate.WeblateHandler(weblateConfig),
            ...
            val metadataApiRouter =
                org.runary.routers.MetadataApiRouter(
                    filters,
                    metadataFetchService,
                    bookService,
            ...
            val audiobookApiRouter =
                org.runary.routers.AudiobookApiRouter(
                    filters,
            ...
            val deviceSyncRouter =
                org.runary.routers.DeviceSyncRouter(
                    filters,
                    org.runary.handlers.KoboSyncHandler(koboSyncService),
                    null,
                    opdsHandler,
                )

            val appHandler =
                org.runary.handlers.AppHandler(
                    fileHandler,
                    config.storage,
                    false,
                    authRouter,
            ...
            val regResp =
                app(
                    Request(Method.POST, "/auth/register")
                        ...
                        .body("""{"username":"ftshttp","email":"fts@http.com","password":"${org.runary.TestPasswords.DEFAULT}"}"""),
                )
