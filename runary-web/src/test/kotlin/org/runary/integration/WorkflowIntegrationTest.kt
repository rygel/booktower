...
class WorkflowIntegrationTest : IntegrationTestBase() {
    ...
    fun `new user first-use workflow - register to reading progress via HTML pages`() {
        ...
        assertTrue(libsPage.contains("Runary"), "Libraries page should be a Runary page")
        ...
        assertTrue(readerResp.bodyString().contains("Runary"), "Reader page should render")
    }
    ...
    fun `JWT remains valid for API calls after password change`() {
        ...
        app(
            Request(Method.POST, "/api/auth/change-password")
                .header("Cookie", "token=$token")
                .header("Content-Type", "application/json")
                .body("""{"currentPassword":"${org.runary.TestPasswords.DEFAULT}","newPassword":"newpassword99"}"""),
        )
