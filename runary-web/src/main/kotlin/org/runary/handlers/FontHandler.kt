...
class FontHandler(
    ...
    fun upload(req: Request): Response {
        ...
        return when (val result = fontService.uploadFont(userId, originalName, bytes)) {
            is kotlin.Result<*> -> {
                if (result.isSuccess) {
                    val f = result.getOrThrow() as org.runary.services.UserFont
                    Response(Status.CREATED)
                        .header("Content-Type", "application/json")
                        .body(
                            Json.mapper.writeValueAsString(
                                mapOf("id" to f.id, "originalName" to f.originalName, "url" to f.url, "fileSize" to f.fileSize),
                            ),
                        )
                } else {
    ...
    fun delete(req: Request): Response {
        ...
        return if (fontService.deleteFont(userId, fontId)) {
