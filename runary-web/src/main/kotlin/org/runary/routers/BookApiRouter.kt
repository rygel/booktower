...
import org.runary.services.CreateNotebookRequest
import org.runary.services.CreateReviewRequest
...
class BookApiRouter(
    ...
    private val communityRatingService: org.runary.services.CommunityRatingService?,
                ...
                Response(Status.OK)
                    ...
                    .body(
                        org.runary.config.Json.mapper
                            .writeValueAsString(mapOf("formats" to org.runary.handlers.SUPPORTED_FORMATS)),
                    )
            },
    ...
    private fun setBookStatus(req: Request): Response {
        ...
        val status =
            statusName?.let { n ->
                org.runary.models.ReadStatus.entries
    ...
    private fun mergeBook(req: Request): Response {
        ...
        val body =
            runCatching {
                org.runary.config.Json.mapper.readValue(
                    req.bodyString(),
                    org.runary.models.MergeBookRequest::class.java,
                )
            }.getOrNull()
    ...
    private fun applyFilenameMetadata(req: Request): Response {
        ...
        val fileMeta =
            org.runary.services.FilenameMetadataService
                .extract(book.filePath ?: book.title)
        val fetchedMeta =
            org.runary.models.FetchedMetadata(
        ...
        val updated =
            bookService.applyFetchedMetadata(userId, bookId, fetchedMeta)
                ?: return Response(Status.NOT_FOUND)
                    .header("Content-Type", "application/json")
                    .body("""{"error":"Book not found"}""")
        fileMeta.series?.let { series ->
            bookService.updateSeries(userId, bookId, series, fileMeta.seriesIndex)
        }
        return Response(Status.OK)
            .header("Content-Type", "application/json")
            .body(
        ...
        val book =
            bookService.getBook(userId, bookId)
                ?: return Response(Status.NOT_FOUND)
                    ...
                    .body("""{"error":"Book not found"}""")
        val sidecar =
            org.runary.services.SidecarMetadataService
                .read(book.filePath ?: "")
                ?: return Response(Status.NOT_FOUND)
                    .header("Content-Type", "application/json")
        ...
        val email =
            body.get("email")?.asText()?.takeIf { it.isNotBlank() }
                ?: return Response(Status.BAD_REQUEST)
                    ...
                    .body("""{"error":"email is required"}""")
        return try {
            val recipient = svc.addRecipient(userId, org.runary.services.AddRecipientRequest(label, email))
    ...
    private fun addBookFile(req: Request): Response {
        ...
        val request =
            runCatching {
                org.runary.config.Json.mapper
                    .readValue(req.bodyString(), org.runary.models.AddBookFileRequest::class.java)
    ...
    private fun updateComicMetadata(req: Request): Response {
        ...
        val request =
            runCatching {
                org.runary.config.Json.mapper
                    .readValue(req.bodyString(), org.runary.models.ComicMetadataRequest::class.java)
    ...
    private fun createShelf(req: Request): Response {
        ...
        val ruleType =
            runCatching {
                org.runary.models.ShelfRuleType
                    .valueOf(ruleTypeStr)
            }.getOrNull()
                ?: return Response(Status.BAD_REQUEST)
        val ruleValue = body.get("ruleValue")?.takeIf { !it.isNull }?.asText()
        val shelf = magicShelfService.createShelf(userId, org.runary.models.CreateMagicShelfRequest(name, ruleType, ruleValue))
    ...
    private fun linkBook(req: Request): Response {
        ...
        val body =
            runCatching {
                org.runary.config.Json.mapper
                    .readValue(req.bodyString(), org.runary.models.LinkBooksRequest::class.java)
            }.getOrNull()
                ?: return Response(Status.BAD_REQUEST)
                    .header("Content-Type", "application/json")
                    .body("""{"error":"Invalid request body"}""")
