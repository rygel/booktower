...
private val logger = LoggerFactory.getLogger("runary.JournalService")
...
class JournalService(
    ...
    fun delete(
        ...
        jdbi.withHandle<Int, Exception> { h ->
            h
                .createUpdate("DELETE FROM book_journal_entries WHERE id = ? AND user_id = ?")
                .bind(0, entryId.toString())
                .bind(1, userId.toString())
                .execute()
        } > 0

    private fun org.jdbi.v3.core.result.RowView.toEntry() =
        JournalEntry(
