...
private val ftsLog = LoggerFactory.getLogger("runary.FtsService")
...
class FtsService(
    ...
    fun initialize() {
        if (!enabled) {
            ftsLog.info("Full-text search disabled (set RUNARY_FTS_ENABLED=true to enable)")
            return
        }
        val isPostgres =
        ...
        if (!isPostgres) {
            ftsLog.warn("RUNARY_FTS_ENABLED=true but database is not PostgreSQL — FTS disabled")
            return
        }
        applyPgSchema()
