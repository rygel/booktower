...
private val logger = LoggerFactory.getLogger("runary.SeedService")
...
class SeedService(
    ...
    private fun downloadComic(
        ...
        try {
            ...
            try {
                ...
                metaConn.setRequestProperty("User-Agent", "Runary/1.0 comic-seed")
            ...
            val result = downloadWithResume(url, destFile, "Runary/1.0 comic-seed", readTimeout = 300_000)
            when (result) {
                "skip" -> {
    ...
    private fun downloadLibrivoxChapters(
        ...
        try {
            ...
            conn.setRequestProperty("User-Agent", "Runary/1.0 librivox-seed")
            if (conn.responseCode != 200) {
    ...
    private fun downloadChapter(
        ...
        return try {
            val result = downloadWithResume(listenUrl, destFile, "Runary/1.0 librivox-seed", readTimeout = 180_000)
            when (result) {
                "skip" -> {
                    false
                }
    ...
    private fun downloadEpub(
        ...
        try {
            ...
            val result = downloadWithResume(url, destFile, "Runary/1.0 gutenberg-seed")
            when (result) {
                "skip" -> {
