...
private val logger = LoggerFactory.getLogger("runary.AuthService")
...
class AuthService(
    ...
    fun register(request: CreateUserRequest): Result<LoginResponse> {
        ...
        val existing =
            jdbi.withHandle<String?, Exception> { handle ->
                handle
                    .createQuery("SELECT id FROM users WHERE username = ?")
    ...
    fun login(request: LoginRequest): Result<LoginResponse> {
        ...
        val user =
            jdbi.withHandle<User, Exception> { handle ->
                handle
                    .createQuery("SELECT * FROM users WHERE username = ? OR email = ?")
                    .bind(0, credential)
                    .bind(1, credential)
                    .map { row -> mapUser(row) }
                    .firstOrNull()
    ...
    fun changePassword(
        ...
        jdbi.useHandle<Exception> { handle ->
            handle
                .createUpdate("UPDATE users SET password_hash = ?, updated_at = ? WHERE id = ?")
                .bind(0, newHash)
    ...
    fun changeEmail(
        ...
        val existing =
            jdbi.withHandle<String?, Exception> { handle ->
                handle
                    .createQuery("SELECT id FROM users WHERE email = ? AND id != ?")
                    .bind(0, trimmedEmail)
        ...
        jdbi.useHandle<Exception> { handle ->
            handle
                .createUpdate("UPDATE users SET email = ?, updated_at = ? WHERE id = ?")
                .bind(0, trimmedEmail)
                .bind(1, Instant.now().toString())
                .bind(2, userId.toString())
                .execute()
    ...
    fun refreshAccessToken(refreshToken: String): Result<LoginResponse> {
        val row =
            jdbi.withHandle<Map<String, String>?, Exception> { handle ->
                handle
                    .createQuery("SELECT user_id, expires_at FROM refresh_tokens WHERE token = ?")
                    .bind(0, refreshToken)
                    .map { r ->
                        mapOf(
    ...
    fun backchannelLogout(sub: String): Int {
        val user =
            jdbi.withHandle<User?, Exception> { h ->
                h
                    .createQuery("SELECT * FROM users WHERE oidc_sub = ?")
                    .bind(0, sub)
                    .map { row -> mapUser(row) }
                    .firstOrNull()
            } ?: return -1
        return jdbi.withHandle<Int, Exception> { h ->
            h
                .createUpdate("DELETE FROM refresh_tokens WHERE user_id = ?")
    ...
    fun getUserById(userId: UUID): User? =
        jdbi.withHandle<User?, Exception> { handle ->
            handle
                .createQuery("SELECT * FROM users WHERE id = ?")
                .bind(0, userId.toString())
                .map { row -> mapUser(row) }
                .firstOrNull()
        }
     ...
     */
    fun findOrCreateOidcUser(
        sub: String,
        email: String?,
        name: String?,
        preferredUsername: String?,
        isAdminByGroup: Boolean = false,
    ): LoginResponse {
        val now = Instant.now()
        ...
        val existing =
            jdbi.withHandle<User?, Exception> { h ->
                ...
                jdbi.withHandle<User?, Exception> { h ->
                    h
                        .createQuery("SELECT * FROM users WHERE email = ?")
                        .bind(0, e)
                        .map { row -> mapUser(row) }
                        .firstOrNull()
                }
        ...
        val user =
            if (existing != null) {
                ...
                jdbi.useHandle<Exception> { h ->
                    h
                        .createUpdate("UPDATE users SET oidc_sub = ?, is_admin = ?, updated_at = ? WHERE id = ?")
                        .bind(0, sub)
                        .bind(1, isAdminByGroup)
                        .bind(2, now.toString())
                        .bind(3, existing.id.toString())
                        .execute()
                }
                ...
                val userId = UUID.randomUUID()
                val username =
                    (preferredUsername ?: name?.replace(" ", "_") ?: email?.substringBefore('@') ?: "oidc_$sub")
                        .take(50)
    ...
    private fun parseTimestamp(value: String): Instant =
        try {
            ...
            LocalDateTime
                .parse(value, DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss[.SSSSSS][.SSSSS][.SSSS][.SSS][.SS][.S]"))
