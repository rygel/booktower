...
private val logger = LoggerFactory.getLogger("runary.PasswordResetService")
    ...
    fun createToken(email: String): String? {
        val userId =
            jdbi.withHandle<String?, Exception> { handle ->
                handle
                    .createQuery("SELECT id FROM users WHERE email = ?")
        ...
        jdbi.useHandle<Exception> { handle ->
            handle
                .createUpdate("DELETE FROM password_reset_tokens WHERE user_id = ? AND used_at IS NULL")
                .bind(0, userId)
                .execute()
        }
    ...
    fun resetPassword(
        ...
        jdbi.useHandle<Exception> { handle ->
            handle
                ...
                .execute()
            handle
                .createUpdate("UPDATE password_reset_tokens SET used_at = ? WHERE id = ?")
                .bind(0, now.toString())
                .bind(1, tokenId)
                .execute()
        }
