    ...
    fun `OPDS catalog authenticated with OPDS credentials succeeds`() {
        ...
        app(
            Request(Method.PUT, "/api/user/opds-credentials")
                .header("Cookie", "token=$token")
                .header("Content-Type", "application/json")
                .body("""{"opdsUsername":"opds_user","password":"${org.runary.TestPasswords.DEFAULT}"}"""),
        )

        val opdsResp =
            app(
                Request(Method.GET, "/opds/catalog")
                    .header("Authorization", basicAuthHeader("opds_user", org.runary.TestPasswords.DEFAULT)),
            )
    ...
    fun `DELETE opds credentials removes them`() {
        ...
        app(
            Request(Method.PUT, "/api/user/opds-credentials")
                .header("Cookie", "token=$token")
                .header("Content-Type", "application/json")
                .body("""{"opdsUsername":"todelete","password":"${org.runary.TestPasswords.DEFAULT}"}"""),
        )
