...
import org.runary.models.ReadingSessionDto
...
class ReadingSessionIntegrationTest : IntegrationTestBase() {
    ...
    fun `sessions are isolated per user`() {
        ...
        app(
            Request(Method.POST, "/ui/books/$bookA/progress")
        ...
        app(
            Request(Method.POST, "/ui/books/$bookB/progress")
        ...
        val sessionsA =
            Json.mapper.readValue(
                app(
                    Request(Method.GET, "/api/books/$bookA/sessions")
                        .header("Cookie", "token=$tokenA"),
                ).bodyString(),
                Array<ReadingSessionDto>::class.java,
            )
        val sessionsB =
            Json.mapper.readValue(
                app(
                    Request(Method.GET, "/api/books/$bookB/sessions")
                        .header("Cookie", "token=$tokenB"),
                ).bodyString(),
                Array<ReadingSessionDto>::class.java,
            )
    ...
    fun `analytics page loads with session data`() {
        ...
        assertTrue(body.contains("Runary"))
    }
