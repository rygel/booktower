...
class ErrorResponseIntegrationTest : IntegrationTestBase() {
    private fun assertJsonError(
        ...
        assertTrue(!body.contains("at org.runary"), "Error response should not leak stack traces")
    ...
    fun `register with duplicate username returns conflict`() {
        ...
        val resp =
            app(
                Request(Method.POST, "/auth/register")
                    .header("Content-Type", "application/json")
                    .body("""{"username":"$username","email":"${username}2@test.com","password":"${org.runary.TestPasswords.DEFAULT}"}"""),
            )
        assertTrue(
