...
class EmailChangeIntegrationTest : IntegrationTestBase() {
    ...
    fun `change email returns 200 with valid credentials`() {
        ...
        val response = changeEmail(token, "new_em1_${System.nanoTime()}@example.com", org.runary.TestPasswords.DEFAULT)
    ...
    fun `change email fails without authentication`() {
        val response =
            app(
                Request(Method.POST, "/api/auth/change-email")
                    .header("Content-Type", "application/json")
                    .body("""{"currentPassword":"${org.runary.TestPasswords.DEFAULT}","newEmail":"x@example.com"}"""),
            )
    ...
    fun `change email fails when email already in use`() {
        ...
        app(
            Request(Method.POST, "/auth/register")
                .header("Content-Type", "application/json")
                .body("""{"username":"em3c_${System.nanoTime()}","email":"$knownEmail","password":"${org.runary.TestPasswords.DEFAULT}"}"""),
        )
        // Try to change token1's email to the already-used email
        val response = changeEmail(token1, knownEmail, org.runary.TestPasswords.DEFAULT)
        assertEquals(Status.BAD_REQUEST, response.status)
        val body = response.bodyString()
    ...
    fun `change email fails with invalid email format`() {
        ...
        val response = changeEmail(token, "not-an-email", org.runary.TestPasswords.DEFAULT)
        assertEquals(Status.BAD_REQUEST, response.status)
    }
    ...
    fun `email is actually updated after change`() {
        ...
        val changeResp = changeEmail(token, newEmail, org.runary.TestPasswords.DEFAULT)
    ...
    fun `change email normalises email to lowercase`() {
        ...
        val response = changeEmail(token, newEmail, org.runary.TestPasswords.DEFAULT)
        assertEquals(Status.OK, response.status)
