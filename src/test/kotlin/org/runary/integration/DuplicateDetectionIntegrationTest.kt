...
class DuplicateDetectionIntegrationTest : IntegrationTestBase() {
    ...
    private fun createLibraryForUser(
        ...
        val pdfMetadataService = org.runary.services.PdfMetadataService(jdbi, TestFixture.config.storage.coversPath)
        val libraryAccessService = org.runary.services.LibraryAccessService(jdbi)
        val libraryService = org.runary.services.LibraryService(jdbi, pdfMetadataService, libraryAccessService)
        val lib = libraryService.createLibrary(userId, org.runary.models.CreateLibraryRequest(name, libDir.absolutePath))
        return lib.id
    }
