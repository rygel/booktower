...
class PhysicalBookIntegrationTest : IntegrationTestBase() {
    ...
    fun `create physical book returns bookFormat=PHYSICAL`() {
        ...
        val resp = app(
            Request(Method.POST, "/api/books")
                ...
                .body("""{"title":"Physical Book","author":"Author","description":null,"libraryId":"$libId","bookFormat":"PHYSICAL"}"""),
        )
    ...
    fun `create audiobook returns bookFormat=AUDIOBOOK`() {
        ...
        assertEquals(Status.CREATED, resp.status)
        assertEquals("AUDIOBOOK", Json.mapper.readTree(resp.bodyString()).get("bookFormat").asText())
    }
    ...
    fun `filter books by format=PHYSICAL returns only physical books`() {
        val token = registerAndGetToken()
        val libId = createLibrary(token)
        app(Request(Method.POST, "/api/books").header("Cookie", "token=$token")
            .header("Content-Type", "application/json")
        ...
        val resp = app(Request(Method.GET, "/api/books?libraryId=$libId&format=PHYSICAL")
            .header("Cookie", "token=$token"))
    ...
    fun `filter books by format=EBOOK excludes physical books`() {
        ...
        val resp = app(Request(Method.GET, "/api/books?libraryId=$libId&format=EBOOK")
            .header("Cookie", "token=$token"))
        assertEquals(Status.OK, resp.status)
    ...
    fun `get book returns bookFormat field`() {
        val token = registerAndGetToken()
        val libId = createLibrary(token)
        val createResp = app(Request(Method.POST, "/api/books").header("Cookie", "token=$token")
            .header("Content-Type", "application/json")
            .body("""{"title":"My Physical Book","author":null,"description":null,"libraryId":"$libId","bookFormat":"PHYSICAL"}"""))
        val bookId = Json.mapper.readTree(createResp.bodyString()).get("id").asText()

        val getResp = app(Request(Method.GET, "/api/books/$bookId").header("Cookie", "token=$token"))
        assertEquals(Status.OK, getResp.status)
        assertEquals("PHYSICAL", Json.mapper.readTree(getResp.bodyString()).get("bookFormat").asText())
    }
