...
class AdditionalMetadataSourcesIntegrationTest : IntegrationTestBase() {
    ...
    fun `GET api metadata sources lists new providers`() {
        ...
        val sources = Json.mapper.readTree(resp.bodyString()).get("sources").map { it.asText() }
    ...
    fun `can request metadata from hardcover source`() {
        ...
        stubResult = FetchedMetadata(
            title = "Test Book", author = "Author", description = "Desc",
            isbn = null, publisher = null, publishedDate = null, source = "hardcover",
            series = "Series A", seriesIndex = 1.0,
        )
        val resp = app(
    ...
    fun `can request metadata from comicvine source`() {
        ...
        capturedSource = "not-called"
        stubResult = FetchedMetadata(
            title = "Batman Vol 1", author = null, description = "Comic desc",
            isbn = null, publisher = "DC Comics", publishedDate = "1986", source = "comicvine",
        )
        val resp = app(
    ...
    fun `can request metadata from audible source`() {
        ...
        capturedSource = "not-called"
        stubResult = FetchedMetadata(
            title = "Dune", author = "Frank Herbert", description = null,
            isbn = null, publisher = "Macmillan Audio", publishedDate = "1987",
            source = "audible", narrator = "Scott Brick", durationSeconds = 21 * 3600,
        )
        val resp = app(
    ...
    fun `FetchedMetadata genres and series fields serialised`() {
        val token = registerAndGetToken()
        stubResult = FetchedMetadata(
            title = "Foundation", author = "Asimov", description = null,
            isbn = null, publisher = null, publishedDate = null, source = "hardcover",
            genres = listOf("Science Fiction", "Classic"),
            series = "Foundation Series", seriesIndex = 1.0,
