...
class ReaderPreferencesIntegrationTest {
    ...
    fun setup() {
        ...
        val appHandler = AppHandler(
            ...
            null, null, null, null, readerPreferencesService,
        )
    ...
    fun `PUT reader-preferences stores preferences for epub`() {
        val token = registerAndToken()
        val resp = app(Request(Method.PUT, "/api/reader-preferences/epub")
    ...
    fun `GET reader-preferences returns previously saved prefs`() {
        val token = registerAndToken()
        app(Request(Method.PUT, "/api/reader-preferences/pdf")
            .header("Cookie", "token=$token").header("Content-Type", "application/json")
    ...
    fun `PATCH reader-preferences merges with existing prefs`() {
        val token = registerAndToken()
        app(Request(Method.PUT, "/api/reader-preferences/cbz")
            .header("Cookie", "token=$token").header("Content-Type", "application/json")
            .body("""{"fitMode":"width","direction":"ltr"}"""))
        val resp = app(Request(Method.PATCH, "/api/reader-preferences/cbz")
            .header("Cookie", "token=$token").header("Content-Type", "application/json")
            .body("""{"direction":"rtl"}"""))
        assertEquals(Status.OK, resp.status)
        val tree = Json.mapper.readTree(resp.bodyString())
