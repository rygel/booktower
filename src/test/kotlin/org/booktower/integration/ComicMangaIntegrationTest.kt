...
class ComicMangaIntegrationTest : IntegrationTestBase() {
    ...
    fun `PUT reading-direction sets rtl on a book`() {
        ...
        val resp = app(Request(Method.PUT, "/api/books/$bookId/reading-direction")
    ...
    fun `reading direction is returned in book GET response`() {
        ...
        app(Request(Method.PUT, "/api/books/$bookId/reading-direction")
    ...
    fun `PUT reading-direction on nonexistent book returns 404`() {
        val token = registerAndGetToken()
        val resp = app(Request(Method.PUT, "/api/books/00000000-0000-0000-0000-000000000000/reading-direction")
            .header("Cookie", "token=$token").header("Content-Type", "application/json")
            .body("""{"direction":"rtl"}"""))
    ...
    fun `PUT reading-direction requires authentication`() {
        val resp = app(Request(Method.PUT, "/api/books/00000000-0000-0000-0000-000000000000/reading-direction")
            .header("Content-Type", "application/json").body("""{"direction":"rtl"}"""))
    ...
    fun `user cannot change reading direction of another user s book`() {
        ...
        val resp = app(Request(Method.PUT, "/api/books/$bookId/reading-direction")
            .header("Cookie", "token=$token2").header("Content-Type", "application/json")
            .body("""{"direction":"rtl"}"""))
