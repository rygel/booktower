...
private val logger = LoggerFactory.getLogger("runary.LibraryHealthService")
    ...
    fun check(userId: UUID): HealthSummary {
        val libs =
            jdbi.withHandle<List<Map<String, Any?>>, Exception> { h ->
                h
                    .createQuery("SELECT id, name, path FROM libraries WHERE user_id = ? ORDER BY name")
            ...
            libs.map { lib ->
                ...
                for (book in books) {
                    ...
                    if (filePath.isBlank() || !File(filePath).exists()) {
                        issues += issue("file_missing")
                    } else if (fileSize == 0L) {
                        issues += issue("zero_size")
                    }
