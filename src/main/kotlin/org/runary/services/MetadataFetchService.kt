...
import org.runary.config.MetadataConfig
...
private val logger = LoggerFactory.getLogger("runary.MetadataFetchService")
...
open class MetadataFetchService(
            ...
            FetchedMetadata(
                ...
                isbn =
                    doc
                        .get("isbn")
                        ?.takeIf { it.isArray && it.size() > 0 }
                        ?.firstOrNull { it.asText().length == 13 }
                        ?.asText()
                        ?: doc
                            .get("isbn")
                            ?.takeIf { it.isArray && it.size() > 0 }
    ...
    private fun fetchFromGoogleBooks(
        title: String,
        author: String?,
    ): FetchedMetadata? {
        return try {
    ...
    private fun fetchFromHardcover(
        title: String,
        author: String?,
    ): FetchedMetadata? {
        val key =
            config.hardcoverApiKey.takeIf { it.isNotBlank() } ?: run {
    ...
    private fun fetchFromComicVine(
        title: String,
        author: String?,
    ): FetchedMetadata? {
        val key =
            config.comicvineApiKey.takeIf { it.isNotBlank() } ?: run {
        ...
        return try {
            ...
            val vol = results[0]
            val coverUrl =
                vol.path("image").textOrNull("medium_url")
    ...
    protected fun JsonNode.textOrNull(field: String): String? = get(field)?.takeIf { it.isTextual && it.asText().isNotBlank() }?.asText()

    private fun Iterable<JsonNode>.firstOrNull(predicate: (JsonNode) -> Boolean): JsonNode? = iterator().asSequence().firstOrNull(predicate)
}
