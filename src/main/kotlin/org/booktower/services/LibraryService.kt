...
class LibraryService(
    ...
    fun getLibrary(userId: UUID, libraryId: UUID): LibraryDto? {
    ...
    private fun mapLibrary(handle: org.jdbi.v3.core.Handle, row: RowView): LibraryDto {
        val libId = UUID.fromString(row.getColumn("id", String::class.java))
        val count = handle.createQuery("SELECT COUNT(*) FROM books WHERE library_id = ?")
    ...
    fun createLibrary(userId: UUID, request: CreateLibraryRequest): LibraryDto {
        val now = Instant.now()
    ...
    fun deleteLibrary(userId: UUID, libraryId: UUID): Boolean {
...
class BookService(private val jdbi: Jdbi, private val storageConfig: StorageConfig) {
    fun getBooks(userId: UUID, libraryId: String?, page: Int = 1, pageSize: Int = 20): BookListDto {
        val offset = (page - 1) * pageSize

        val books = if (libraryId != null) {
    ...
    fun getBook(userId: UUID, bookId: UUID): BookDto? {
    ...
    fun getRecentBooks(userId: UUID, limit: Int = 10): List<BookDto> {
    ...
    fun createBook(userId: UUID, request: CreateBookRequest): Result<BookDto> {
        val libId = UUID.fromString(request.libraryId)
        
        val exists = jdbi.withHandle<String?, Exception> { handle ->
        ...
        jdbi.useHandle<Exception> { handle ->
            handle.createUpdate("INSERT INTO books (id, library_id, title, author, description, file_path, file_size, added_at, updated_at) VALUES (?, ?, ?, ?, ?, '', 0, ?, ?)")
                .bind(0, bookId.toString())
                .bind(1, libId.toString())
    ...
    fun deleteBook(userId: UUID, bookId: UUID): Boolean {
    ...
    fun updateProgress(userId: UUID, bookId: UUID, request: UpdateProgressRequest): ReadingProgressDto? {
        ...
        val existing = jdbi.withHandle<ReadingProgressDto?, Exception> { handle ->
            handle.createQuery("SELECT * FROM reading_progress WHERE user_id = ? AND book_id = ?")
                ...
                .map { row ->
                    ReadingProgressDto(
                        ...
                        totalPages = row.getColumn("total_pages", Int::class.java).takeIf { row.getColumn("total_pages", Any::class.java) != null },
                        percentage = row.getColumn("percentage", Double::class.java).takeIf { row.getColumn("percentage", Any::class.java) != null },
        ...
        if (existing != null) {
            jdbi.useHandle<Exception> { handle ->
                handle.createUpdate("UPDATE reading_progress SET current_page = ?, percentage = ?, last_read_at = ? WHERE user_id = ? AND book_id = ?")
            ...
            jdbi.useHandle<Exception> { handle ->
                handle.createUpdate("INSERT INTO reading_progress (id, user_id, book_id, current_page, total_pages, percentage, last_read_at, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)")
