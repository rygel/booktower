...
class MagicShelfService(
    ...
    fun getShelves(userId: UUID): List<MagicShelfDto> {
        val shelves = jdbi.withHandle<List<MagicShelfDto>, Exception> { handle ->
            handle.createQuery("SELECT * FROM magic_shelves WHERE user_id = ? ORDER BY name")
    ...
    fun getShelf(userId: UUID, shelfId: UUID): MagicShelfDto? {
        val shelf = jdbi.withHandle<MagicShelfDto?, Exception> { handle ->
            handle.createQuery("SELECT * FROM magic_shelves WHERE id = ? AND user_id = ?")
    ...
    fun createShelf(userId: UUID, request: CreateMagicShelfRequest): MagicShelfDto {
    ...
    fun deleteShelf(userId: UUID, shelfId: UUID): Boolean {
        val deleted = jdbi.withHandle<Int, Exception> { handle ->
            handle.createUpdate("DELETE FROM magic_shelves WHERE id = ? AND user_id = ?")
                .bind(0, shelfId.toString())
                .bind(1, userId.toString())
                .execute()
        }
    ...
    fun resolveBooks(userId: UUID, shelf: MagicShelfDto): List<BookDto> = when (shelf.ruleType) {
    ...
    private fun countMatchingBooks(userId: UUID, shelf: MagicShelfDto): Int = when (shelf.ruleType) {
        ShelfRuleType.STATUS -> jdbi.withHandle<Int, Exception> { handle ->
            handle.createQuery(
        ...
        ShelfRuleType.TAG -> jdbi.withHandle<Int, Exception> { handle ->
            handle.createQuery(
                """SELECT COUNT(*) FROM books b
                   INNER JOIN libraries l ON b.library_id = l.id
