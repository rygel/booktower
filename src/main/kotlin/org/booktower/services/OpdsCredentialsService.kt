...
class OpdsCredentialsService(
    ...
    fun getCredentials(userId: UUID): OpdsCredentialsInfo? =
        jdbi.withHandle<OpdsCredentialsInfo?, Exception> { h ->
            h
                .createQuery("SELECT opds_username FROM opds_credentials WHERE user_id = ?")
                .bind(0, userId.toString())
                .map { row ->
                    OpdsCredentialsInfo(
                        userId = userId,
    ...
    fun clearCredentials(userId: UUID): Boolean {
        val rows =
            jdbi.withHandle<Int, Exception> { h ->
                h
                    .createUpdate("DELETE FROM opds_credentials WHERE user_id = ?")
                    .bind(0, userId.toString())
                    .execute()
            }
        return rows > 0
    ...
    fun authenticate(
        ...
        val row =
            jdbi.withHandle<Pair<String, String>?, Exception> { h ->
                h
                    .createQuery("SELECT user_id, password_hash FROM opds_credentials WHERE opds_username = ?")
                    .bind(0, opdsUsername)
                    .map { r ->
                        Pair(
