...
data class PendingHashBook(val bookId: String, val filePath: String)
...
class ComicPageHashService(
    ...
    fun enqueue(bookId: String) {
        jdbi.useHandle<Exception> { h ->
            val exists = h.createQuery("SELECT COUNT(*) FROM comic_hash_queue WHERE book_id = ?")
                .bind(0, bookId).mapTo(Int::class.java).one() > 0
            if (!exists) {
                h.createUpdate(
                    "INSERT INTO comic_hash_queue (book_id, status, queued_at) VALUES (?, 'pending', ?)",
                ).bind(0, bookId).bind(1, Instant.now().toString()).execute()
            }
        }
    }
    ...
    fun indexBook(bookId: String, filePath: String) {
        ...
        jdbi.useHandle<Exception> { h ->
            h.createUpdate("DELETE FROM comic_page_hashes WHERE book_id = ?")
            ...
            for (page in pages) {
                ...
                ).bind(0, bookId).bind(1, page.index).bind(2, hash).execute()
                hashed++
            }
    ...
    fun markStatus(bookId: String, status: String, error: String? = null) {
        jdbi.useHandle<Exception> { h ->
            h.createUpdate(
                "UPDATE comic_hash_queue SET status = ?, indexed_at = ?, error_msg = ? WHERE book_id = ?",
            ).bind(0, status).bind(1, Instant.now().toString()).bind(2, error).bind(3, bookId).execute()
        }
    }
    ...
    fun findDuplicatePages(userId: UUID): List<PageDuplicateGroup> {
        val rows = jdbi.withHandle<List<Map<String, Any?>>, Exception> { h ->
            h.createQuery(
                """SELECT h1.phash,
                   ...
                   ORDER BY h1.phash""",
            ).bind(0, userId.toString()).bind(1, userId.toString()).mapToMap().list()
        }
    ...
    fun countByStatus(): Map<String, Int> =
        jdbi.withHandle<Map<String, Int>, Exception> { h ->
            h.createQuery("SELECT status, COUNT(*) AS cnt FROM comic_hash_queue GROUP BY status")
                .map { row ->
