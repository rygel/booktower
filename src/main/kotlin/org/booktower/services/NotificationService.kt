...
class NotificationService(
    ...
    fun markRead(
        ...
        val rows =
            jdbi.withHandle<Int, Exception> { h ->
                h
                    .createUpdate("UPDATE notifications SET is_read = TRUE WHERE id = ? AND user_id = ?")
                    .bind(0, notificationId)
                    .bind(1, userId.toString())
                    .execute()
            }
        return rows > 0
    ...
    fun markAllRead(userId: UUID): Int =
        jdbi.withHandle<Int, Exception> { h ->
            h
                .createUpdate("UPDATE notifications SET is_read = TRUE WHERE user_id = ? AND is_read = FALSE")
                .bind(0, userId.toString())
                .execute()
        }
    ...
    fun delete(
        ...
        val rows =
            jdbi.withHandle<Int, Exception> { h ->
                h
                    .createUpdate("DELETE FROM notifications WHERE id = ? AND user_id = ?")
                    .bind(0, notificationId)
                    .bind(1, userId.toString())
                    .execute()
            }
    ...
    fun unreadCount(userId: UUID): Int =
        jdbi.withHandle<Int, Exception> { h ->
            h
                .createQuery("SELECT COUNT(*) FROM notifications WHERE user_id = ? AND is_read = FALSE")
                .bind(0, userId.toString())
