...
data class ListeningHeatmapEntry(val date: String, val secondsListened: Int)
...
class ListeningStatsService(private val jdbi: Jdbi) {

    fun getStats(userId: UUID, days: Int = 365): ListeningStats {
        val since = LocalDate.now().minusDays(days.toLong())
        val sinceStr = since.toString()

        val totalSeconds = jdbi.withHandle<Long, Exception> { h ->
            h.createQuery("SELECT COALESCE(SUM(seconds_listened), 0) FROM listen_sessions WHERE user_id = ?")
        ...
        val totalSessions = jdbi.withHandle<Int, Exception> { h ->
            h.createQuery("SELECT COUNT(*) FROM listen_sessions WHERE user_id = ?")
                .bind(0, userId.toString()).mapTo(Int::class.java).firstOrNull() ?: 0
        }
        ...
        val secondsByTag = jdbi.withHandle<Map<String, Long>, Exception> { h ->
            h.createQuery(
        ...
        val secondsByCategory = jdbi.withHandle<Map<String, Long>, Exception> { h ->
            h.createQuery(
