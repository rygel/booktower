...
data class LibraryAccessEntry(val libraryId: String, val grantedAt: String)

class LibraryAccessService(private val jdbi: Jdbi) {
    ...
    fun isRestricted(userId: UUID): Boolean {
        return jdbi.withHandle<Boolean, Exception> { h ->
            h.createQuery("SELECT is_library_restricted FROM users WHERE id = ?")
                .bind(0, userId.toString())
                .mapTo(java.lang.Boolean::class.java)
                .firstOrNull()?.booleanValue() ?: false
        }
    }

    /** Sets restricted mode on a user. */
    fun setRestricted(userId: UUID, restricted: Boolean) {
        jdbi.useHandle<Exception> { h ->
            h.createUpdate("UPDATE users SET is_library_restricted = ? WHERE id = ?")
                .bind(0, restricted)
    ...
    fun listGrantedLibraries(userId: UUID): List<LibraryAccessEntry> {
        return jdbi.withHandle<List<LibraryAccessEntry>, Exception> { h ->
            h.createQuery("SELECT library_id, granted_at FROM user_library_access WHERE user_id = ? ORDER BY granted_at")
                .bind(0, userId.toString())
                .map { row ->
                    LibraryAccessEntry(
    ...
    fun grantAccess(userId: UUID, libraryId: UUID) {
        val exists = jdbi.withHandle<Boolean, Exception> { h ->
            h.createQuery("SELECT COUNT(*) FROM user_library_access WHERE user_id = ? AND library_id = ?")
                .bind(0, userId.toString()).bind(1, libraryId.toString())
    ...
    fun revokeAccess(userId: UUID, libraryId: UUID) {
        jdbi.useHandle<Exception> { h ->
            h.createUpdate("DELETE FROM user_library_access WHERE user_id = ? AND library_id = ?")
                .bind(0, userId.toString())
                .bind(1, libraryId.toString())
                .execute()
        }
