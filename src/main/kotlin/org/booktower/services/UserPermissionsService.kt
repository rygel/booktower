...
class UserPermissionsService(private val jdbi: Jdbi) {

    /** Returns permissions for a user, creating default row if not yet set. */
    fun getPermissions(userId: UUID): UserPermissions {
        val existing = jdbi.withHandle<UserPermissions?, Exception> { h ->
            h.createQuery("SELECT * FROM user_permissions WHERE user_id = ?")
    ...
    fun setPermissions(userId: UUID, permissions: UserPermissions): UserPermissions {
        val existing = jdbi.withHandle<Boolean, Exception> { h ->
            h.createQuery("SELECT COUNT(*) FROM user_permissions WHERE user_id = ?")
                .bind(0, userId.toString())
    ...
    private fun insertPermissions(p: UserPermissions) {
        jdbi.useHandle<Exception> { h ->
            ...
            """)
                .bind(0, p.userId)
                .bind(1, p.canManageLibraries).bind(2, p.canUploadBooks).bind(3, p.canDownloadBooks)
                .bind(4, p.canDeleteBooks).bind(5, p.canEditMetadata).bind(6, p.canManageBookmarks)
                .bind(7, p.canManageAnnotations).bind(8, p.canManageReadingProgress).bind(9, p.canManageShelves)
                .bind(10, p.canExportBooks).bind(11, p.canSendToDevice).bind(12, p.canUseKoboSync)
                .bind(13, p.canUseKoreaderSync).bind(14, p.canUseOpds).bind(15, p.canUseApiTokens)
                .bind(16, p.canManageJournal).bind(17, p.canManageReadingSessions).bind(18, p.canViewStats)
                .bind(19, p.canEditProfile).bind(20, p.canChangePassword).bind(21, p.canChangeEmail)
                .bind(22, p.canUseSearchFilters).bind(23, p.canViewAuditLog).bind(24, p.canAccessKomgaApi)
                .bind(25, p.canManageNotebooks).bind(26, p.canManageFonts).bind(27, p.canAccessAdminPanel)
    ...
    private fun mapPermissions(row: RowView): UserPermissions {
        ...
        fun boolD(col: String, default: Boolean): Boolean {
            return try { row.getColumn(col, java.lang.Boolean::class.java)?.booleanValue() ?: default }
...
private fun org.jdbi.v3.core.statement.Update.bindAll(p: UserPermissions, userId: UUID): org.jdbi.v3.core.statement.Update {
    return this
        .bind(0, p.canManageLibraries).bind(1, p.canUploadBooks).bind(2, p.canDownloadBooks)
        .bind(3, p.canDeleteBooks).bind(4, p.canEditMetadata).bind(5, p.canManageBookmarks)
        .bind(6, p.canManageAnnotations).bind(7, p.canManageReadingProgress).bind(8, p.canManageShelves)
        .bind(9, p.canExportBooks).bind(10, p.canSendToDevice).bind(11, p.canUseKoboSync)
        .bind(12, p.canUseKoreaderSync).bind(13, p.canUseOpds).bind(14, p.canUseApiTokens)
        .bind(15, p.canManageJournal).bind(16, p.canManageReadingSessions).bind(17, p.canViewStats)
        .bind(18, p.canEditProfile).bind(19, p.canChangePassword).bind(20, p.canChangeEmail)
        .bind(21, p.canUseSearchFilters).bind(22, p.canViewAuditLog).bind(23, p.canAccessKomgaApi)
        .bind(24, p.canManageNotebooks).bind(25, p.canManageFonts).bind(26, p.canAccessAdminPanel)
