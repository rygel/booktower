...
private val logger = LoggerFactory.getLogger("booktower.TelemetryService")
...
class TelemetryService(
    ...
    fun isOptedIn(userId: UUID): Boolean =
        userSettingsService.get(userId, TELEMETRY_OPT_IN_KEY) == "true"
     ...
     */
    fun record(userId: UUID, eventType: String, payload: String? = null) {
        ...
        try {
            jdbi.useHandle<Exception> { h ->
                ...
                ).bind(0, id).bind(1, eventType).bind(2, payload).bind(3, now).execute()
            }
        } catch (e: Exception) {
    ...
    fun getStats(): TelemetryStatsDto {
        val byType = jdbi.withHandle<Map<String, Int>, Exception> { h ->
            h.createQuery("SELECT event_type, COUNT(*) as cnt FROM telemetry_events GROUP BY event_type")
                .map { row ->
