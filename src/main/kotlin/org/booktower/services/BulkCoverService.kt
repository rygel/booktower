...
private val logger = LoggerFactory.getLogger("booktower.BulkCoverService")
...
class BulkCoverService(
     ...
     */
    fun regenerateCovers(userId: UUID, libraryId: String?): BulkCoverResult {
        val books = jdbi.withHandle<List<Map<String, Any?>>, Exception> { h ->
            val query = if (libraryId != null) {
                h.createQuery(
                ...
                ).bind(0, userId.toString()).bind(1, libraryId)
            } else {
                h.createQuery(
        ...
        for (book in books) {
            ...
            try {
                when (file.extension.lowercase()) {
                    "pdf" -> { pdfMetadataService.submitAsync(bookId, file); submitted++ }
                    "epub" -> { epubMetadataService.submitAsync(bookId, file); submitted++ }
                    else -> skipped++
                }
            } catch (e: Exception) {
