...
private val deliveryLogger = LoggerFactory.getLogger("booktower.BookDeliveryService")
...
data class AddRecipientRequest(val label: String, val email: String)
...
class BookDeliveryService(
    ...
    fun addRecipient(userId: UUID, req: AddRecipientRequest): EmailRecipient {
        ...
        jdbi.useHandle<Exception> { h ->
            h.createUpdate(
                ...
                .bind(0, id).bind(1, userId.toString()).bind(2, req.label.trim())
                .bind(3, req.email.trim().lowercase()).bind(4, now).execute()
        }
        return EmailRecipient(id, userId.toString(), req.label.trim(), req.email.trim().lowercase(), now)
    }

    fun deleteRecipient(userId: UUID, recipientId: String): Boolean =
        jdbi.withHandle<Int, Exception> { h ->
            h.createUpdate("DELETE FROM email_recipients WHERE id = ? AND user_id = ?")
                .bind(0, recipientId).bind(1, userId.toString()).execute()
        } > 0
     ...
     */
    fun sendBook(userId: UUID, bookId: UUID, toEmail: String): Boolean {
        require(toEmail.contains('@')) { "invalid email address" }
        val book = bookService.getBook(userId, bookId)
            ?: throw IllegalArgumentException("Book not found")
        val filePath = book.filePath?.takeIf { it.isNotBlank() }
            ?: throw IllegalArgumentException("Book has no file attached")
        val file = File(filePath).takeIf { it.exists() && it.isFile }
