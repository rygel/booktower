...
class BookNotebookService(private val jdbi: Jdbi) {

    fun listForBook(bookId: String, userId: UUID): List<NotebookDto> =
        jdbi.withHandle<List<NotebookDto>, Exception> { h ->
            ...
            ).bind(0, bookId).bind(1, userId.toString()).map { row -> mapRow(row) }.list()
        }

    fun get(bookId: String, notebookId: String, userId: UUID): NotebookDto? =
        jdbi.withHandle<NotebookDto?, Exception> { h ->
            ...
            ).bind(0, notebookId).bind(1, bookId).bind(2, userId.toString()).map { row -> mapRow(row) }.firstOrNull()
        }

    fun create(bookId: String, userId: UUID, request: CreateNotebookRequest): NotebookDto {
        ...
        jdbi.useHandle<Exception> { h ->
            h.createUpdate(
                ...
                .bind(3, request.title).bind(4, request.content).bind(5, now).bind(6, now).execute()
        }
        return NotebookDto(id = id, bookId = bookId, userId = userId.toString(), title = request.title, content = request.content, createdAt = now, updatedAt = now)
    }

    fun update(bookId: String, notebookId: String, userId: UUID, request: UpdateNotebookRequest): NotebookDto? {
        get(bookId, notebookId, userId) ?: return null
        val now = Instant.now().toString()
        val sets = mutableListOf<String>()
        val bindings = mutableListOf<Any?>()
        request.title?.let { require(it.isNotBlank()) { "Title must not be blank" }; sets += "title = ?"; bindings += it }
        request.content?.let { sets += "content = ?"; bindings += it }
    ...
    fun delete(bookId: String, notebookId: String, userId: UUID): Boolean {
        val rows = jdbi.withHandle<Int, Exception> { h ->
            h.createUpdate("DELETE FROM book_notebooks WHERE id = ? AND book_id = ? AND user_id = ?")
                .bind(0, notebookId).bind(1, bookId).bind(2, userId.toString()).execute()
        }
        return rows > 0
    }

    private fun mapRow(row: org.jdbi.v3.core.result.RowView) = NotebookDto(
