...
open class EmailService(val config: SmtpConfig) {

    private val i18n = I18nService.create("messages")

    private val mailer: Mailer? = if (config.enabled) {
    ...
    open fun sendBook(toEmail: String, bookTitle: String, filename: String, fileBytes: ByteArray) {
        if (mailer == null) {
