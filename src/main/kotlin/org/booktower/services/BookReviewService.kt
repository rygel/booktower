...
class BookReviewService(private val jdbi: Jdbi) {
    ...
    fun listForBook(bookId: String): List<BookReviewDto> =
        jdbi.withHandle<List<BookReviewDto>, Exception> { h ->
            ...
            ).bind(0, bookId).map { row -> mapRow(row) }.list()
        }

    fun getForUser(bookId: String, userId: UUID): BookReviewDto? =
        jdbi.withHandle<BookReviewDto?, Exception> { h ->
            ...
            ).bind(0, bookId).bind(1, userId.toString()).map { row -> mapRow(row) }.firstOrNull()
        }

    fun create(bookId: String, userId: UUID, request: CreateReviewRequest): BookReviewDto {
        ...
        jdbi.useHandle<Exception> { h ->
            ...
            ).bind(0, id).bind(1, bookId).bind(2, userId.toString())
                .bind(3, request.rating).bind(4, request.title).bind(5, request.body)
                .bind(6, request.spoiler).bind(7, now).bind(8, now).execute()
        }
        return BookReviewDto(
            id = id, bookId = bookId, userId = userId.toString(),
            rating = request.rating, title = request.title, body = request.body,
            spoiler = request.spoiler, createdAt = now, updatedAt = now,
        )
    }

    fun update(bookId: String, reviewId: String, userId: UUID, request: UpdateReviewRequest): BookReviewDto? {
        val existing = jdbi.withHandle<BookReviewDto?, Exception> { h ->
            h.createQuery(
            ...
            ).bind(0, reviewId).bind(1, bookId).bind(2, userId.toString()).map { row -> mapRow(row) }.firstOrNull()
        } ?: return null
        ...
        request.rating?.let { sets += "rating = ?"; bindings += it } ?: run { sets += "rating = ?"; bindings += existing.rating }
        request.title?.let { sets += "title = ?"; bindings += it }
        request.body?.let { sets += "body = ?"; bindings += it }
        request.spoiler?.let { sets += "spoiler = ?"; bindings += it }
        ...
        return jdbi.withHandle<BookReviewDto?, Exception> { h ->
            ...
            ).bind(0, reviewId).map { row -> mapRow(row) }.firstOrNull()
        }
    }

    fun delete(bookId: String, reviewId: String, userId: UUID): Boolean {
        val rows = jdbi.withHandle<Int, Exception> { h ->
            h.createUpdate("DELETE FROM book_reviews WHERE id = ? AND book_id = ? AND user_id = ?")
                .bind(0, reviewId).bind(1, bookId).bind(2, userId.toString()).execute()
        }
        return rows > 0
    }

    private fun mapRow(row: org.jdbi.v3.core.result.RowView) = BookReviewDto(
