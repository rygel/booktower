...
object SearchNormalizer {
    ...
    fun containsCjk(query: String): Boolean = query.any { c ->
        c in '\u3000'..'\u9FFF' ||   // CJK Unified, Hiragana, Katakana, CJK Symbols
    ...
    fun toHalfWidth(s: String): String = s.map { c ->
        when {
            c == '\u3000' -> ' '
    ...
    fun katakanaToHiragana(s: String): String = s.map { c ->
        if (c in '\u30A1'..'\u30F6') (c.code - 0x60).toChar() else c
    }.joinToString("")

    /** Hiragana (U+3041–U+3096) → Katakana (U+30A1–U+30F6). */
    fun hiraganaToKatakana(s: String): String = s.map { c ->
