...
data class FilterPresetDto(
    ...
    val userId: String,
    val name: String,
    val filters: String,   // JSON blob — the client decides the schema
...
data class SaveFilterPresetRequest(
    val name: String,
    val filters: JsonNode,  // accepts any JSON object/value from client
) {
    val filtersAsString: String get() = mapper.writeValueAsString(filters)
}

class FilterPresetService(private val jdbi: Jdbi) {
    ...
    fun list(userId: UUID): List<FilterPresetDto> =
        jdbi.withHandle<List<FilterPresetDto>, Exception> { h ->
            ...
            ).bind(0, userId.toString()).map { row -> mapRow(row) }.list()
        }

    fun get(userId: UUID, presetId: String): FilterPresetDto? =
        jdbi.withHandle<FilterPresetDto?, Exception> { h ->
            ...
            ).bind(0, presetId).bind(1, userId.toString()).map { row -> mapRow(row) }.firstOrNull()
        }

    fun create(userId: UUID, request: SaveFilterPresetRequest): FilterPresetDto {
        ...
        jdbi.useHandle<Exception> { h ->
            ...
            ).bind(0, id).bind(1, userId.toString()).bind(2, request.name).bind(3, request.filtersAsString)
                .bind(4, now).bind(5, now).execute()
        }
        return FilterPresetDto(id = id, userId = userId.toString(), name = request.name, filters = request.filtersAsString, createdAt = now, updatedAt = now)
    }

    fun update(userId: UUID, presetId: String, request: SaveFilterPresetRequest): FilterPresetDto? {
        ...
        jdbi.useHandle<Exception> { h ->
            ...
            ).bind(0, request.name).bind(1, request.filtersAsString).bind(2, now).bind(3, presetId).bind(4, userId.toString()).execute()
        }
        return get(userId, presetId)
    }

    fun delete(userId: UUID, presetId: String): Boolean {
        val rows = jdbi.withHandle<Int, Exception> { h ->
            h.createUpdate("DELETE FROM filter_presets WHERE id = ? AND user_id = ?")
                .bind(0, presetId).bind(1, userId.toString()).execute()
        }
        return rows > 0
    }

    private fun mapRow(row: org.jdbi.v3.core.result.RowView) = FilterPresetDto(
