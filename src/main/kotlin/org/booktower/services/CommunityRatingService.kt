...
open class CommunityRatingService(private val jdbi: Jdbi) {

    protected open val http: HttpClient = HttpClient.newBuilder()
     ...
     */
    fun fetchAndStore(userId: UUID, bookId: UUID): CommunityRatingDto? {
        val book = jdbi.withHandle<Map<String, String?>, Exception> { h ->
            h.createQuery("""
        ...
        val result = fetchFromGoogleBooks(book["googleBooksId"], book["isbn"], book["title"])
        ...
        if (result != null) {
            ...
            jdbi.useHandle<Exception> { h ->
                h.createUpdate("""
                    ...
                    WHERE id = ?
                """).bind(0, result.rating).bind(1, result.count)
                    .bind(2, result.source).bind(3, now).bind(4, bookId.toString()).execute()
            }
    ...
    fun getStored(userId: UUID, bookId: UUID): CommunityRatingDto? {
        return jdbi.withHandle<CommunityRatingDto?, Exception> { h ->
            h.createQuery("""
    ...
    protected open fun fetchFromGoogleBooks(googleBooksId: String?, isbn: String?, title: String?): CommunityRatingDto? {
        val url = when {
        ...
        return runCatching {
            ...
            val volumeInfo = if (json.has("items")) json.get("items")?.get(0)?.get("volumeInfo")
                             else json.get("volumeInfo")
        ...
        }.getOrElse { e -> logger.debug("Google Books rating fetch failed: ${e.message}"); null }
    }

    protected open fun fetchFromOpenLibrary(openLibraryId: String?, isbn: String?): CommunityRatingDto? {
        val worksId = when {
        ...
        }.getOrElse { e -> logger.debug("OpenLibrary rating fetch failed: ${e.message}"); null }
    }
