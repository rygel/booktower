...
data class SeedResult(val libraries: Int, val books: Int)
data class SeedFilesResult(val queued: Int, val skipped: Int)
...
private val LIBRIVOX_SEED_BOOKS = listOf(
    SeedAudioBook(
...
private data class SeedLibrary(val name: String, val books: List<SeedBook>)

private val SEED_LIBRARIES = listOf(

    SeedLibrary("Science Fiction Classics", listOf(
        SeedBook(
    ...
    SeedLibrary("Victorian & Classic Literature", listOf(
        SeedBook(
    ...
    SeedLibrary("Adventure & Philosophy", listOf(
        SeedBook(
            title = "Treasure Island",
            author = "Robert Louis Stevenson",
...
private fun normalizeSeedDate(date: String?): String? = when {
    date == null -> null
...
class SeedService(
    ...
    fun seed(userId: UUID): SeedResult? {
        ...
        SEED_LIBRARIES.forEach { seedLib ->
            ...
            val lib = libraryService.createLibrary(
                userId,
    ...
    fun seedFiles(userId: UUID): SeedFilesResult? {
        ...
        SEED_LIBRARIES.forEach { seedLib ->
            ...
            seedLib.books.forEach { seedBook ->
                if (seedBook.gutenbergId == null) { skipped++; return@forEach }
                val book = books.firstOrNull { it.title == seedBook.title } ?: return@forEach
                if (book.fileSize > 0) { skipped++; return@forEach } // already has a file
            ...
            bookService.createBook(
                ...
                bookService.updateBook(
                    ...
                    UpdateBookRequest(
                        ...
                        author = seedBook.author,
                        description = seedBook.description,
                        series = null, seriesIndex = null, isbn = null,
                        publisher = "LibriVox", publishedDate = null, pageCount = null,
                    ),
                )
    ...
    private fun downloadEpub(userId: UUID, bookId: UUID, gutenbergId: Int, title: String) {
        try {
