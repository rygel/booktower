...
class ExportService(private val jdbi: Jdbi) {

    fun exportUser(userId: UUID): UserExportDto {
        val uid = userId.toString()

        val user = jdbi.withHandle<Triple<String, String, String>?, Exception> { handle ->
            handle.createQuery("SELECT username, email, created_at FROM users WHERE id = ?")
                .bind(0, uid)
                .map { r ->
                    Triple(
        ...
        val libraries = jdbi.withHandle<List<Triple<String, String, String>>, Exception> { handle ->
            handle.createQuery("SELECT id, name, path, created_at FROM libraries WHERE user_id = ? ORDER BY created_at")
                .bind(0, uid)
                .map { r ->
                    Triple(
        ...
        val libraryDtos = libraries.map { (libId, libName, pathAndDate) ->
    ...
    private fun exportBooks(userId: UUID, libraryId: String): List<BookExportDto> {
        val uid = userId.toString()
        ...
        val books = jdbi.withHandle<List<BookRow>, Exception> { handle ->
            handle.createQuery(
        ...
        return books.map { book ->
            val status = jdbi.withHandle<String?, Exception> { handle ->
                handle.createQuery("SELECT status FROM book_status WHERE user_id = ? AND book_id = ?")
                    .bind(0, uid).bind(1, book.id)
                    .mapTo(String::class.java).firstOrNull()
            }
            val rating = jdbi.withHandle<Int?, Exception> { handle ->
                handle.createQuery("SELECT rating FROM book_ratings WHERE user_id = ? AND book_id = ?")
                    .bind(0, uid).bind(1, book.id)
                    .mapTo(java.lang.Integer::class.java).firstOrNull()?.toInt()
            }
            val tags = jdbi.withHandle<List<String>, Exception> { handle ->
                handle.createQuery("SELECT tag FROM book_tags WHERE user_id = ? AND book_id = ? ORDER BY tag")
                    .bind(0, uid).bind(1, book.id)
                    .mapTo(String::class.java).list()
            }
            val progress = jdbi.withHandle<ProgressExportDto?, Exception> { handle ->
                handle.createQuery(
            ...
            val bookmarks = jdbi.withHandle<List<BookmarkExportDto>, Exception> { handle ->
                handle.createQuery(
