...
class KoboSyncHandler(private val koboSyncService: KoboSyncService) {
    ...
    fun deleteDevice(req: Request): Response {
        ...
        return if (koboSyncService.deleteDevice(userId, token)) Response(Status.NO_CONTENT)
    ...
    fun readingState(req: Request): Response {
        val token = req.path("token") ?: return Response(Status.UNAUTHORIZED)
        val device = koboSyncService.getDevice(token) ?: return Response(Status.UNAUTHORIZED)
        val bookId = req.path("bookId")?.let { runCatching { UUID.fromString(it) }.getOrNull() }
