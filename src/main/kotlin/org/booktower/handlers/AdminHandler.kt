...
class AdminHandler(
    ...
    fun generateResetLink(req: Request): Response {
        ...
        val token = passwordResetService.createToken(user.email)
    ...
    fun findDuplicates(req: Request): Response {
        val svc = duplicateDetectionService
    ...
    private fun actorName(req: Request): String =
        req.header("X-Auth-Username") ?: AuthenticatedUser.from(req).toString()
    ...
    fun grantLibraryAccess(req: Request): Response {
        ...
        val libUuid = try { java.util.UUID.fromString(libraryId) } catch (_: Exception) { return badRequest("Invalid libraryId") }
    ...
    fun revokeLibraryAccess(req: Request): Response {
        ...
        val userId = try { java.util.UUID.fromString(parts[parts.size - 3]) } catch (_: Exception) { return badRequest("Invalid user ID") }
        val libraryId = try { java.util.UUID.fromString(parts.last()) } catch (_: Exception) { return badRequest("Invalid libraryId") }
